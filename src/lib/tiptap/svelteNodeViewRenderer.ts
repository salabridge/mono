...
class SvelteNodeView extends NodeView<SvelteComponentRaw, Editor, SvelteNodeViewRendererOptions> {
  ...
  override get dom() {
    console.info('what', this.renderer);
    if (this.renderer && !this?.renderer.dom.firstElementChild?.hasAttribute('data-node-view-wrapper')) {
  ...
  update(node: ProseMirrorNode, decorations: DecorationWithType[]): boolean {
    console.info('svelte node view renderer update called');
    const updateProps = () => {
